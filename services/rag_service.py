...
字段: {fields_info}"""
    ...
    def _build_context(self, search_results: List[Dict]) -> str:
        ...
        for i, result in enumerate(search_results[:3], 1):
            meta = result['metadata']
            context += f"表{i}：{meta['table_name_en']}\n"
            context += f"- 资源名称: {meta['resource_name']}\n"
            context += f"- 领域分类: {meta['domain_category']}\n"
            context += f"- 组织机构: {meta['organization_name']}\n"
            context += f"- 数据分层: {meta['layer']}\n\n"
